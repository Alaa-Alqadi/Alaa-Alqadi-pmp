...
const Sidebar: React.FC<SidebarProps> = ({ projects, selectedProjectId, onSelectProject, onAddProject, onManageTeam, searchTerm, onSearchChange, currentUser, onLogout }) => {
  ...
  return (
    <aside className="w-64 bg-slate-900 p-4 flex flex-col border-s border-slate-800 rtl:border-s-0 rtl:border-e">
      <div>
          ...
          {sortedClients.map((clientName) => (
            <li key={clientName} className="mt-1 first:mt-0">
              <button
                ...
                className="w-full flex justify-between items-center px-3 py-2 text-sm font-semibold text-slate-300 hover:bg-slate-750 rounded-md transition-colors text-start"
        ...
        <div className="p-2 bg-slate-850 rounded-lg">
            <div className="flex items-center">
                ...
                <button 
                    ...
                    className="ms-auto p-2 text-slate-400 hover:text-white hover:bg-slate-700 rounded-md transition-colors"
